...
import {
  Plus, Trash2, ImagePlus, FolderKanban, FileText, X,
...
function loadProjects(): ProjectDoc[] {
  try {
...
function saveProjects(docs: ProjectDoc[]) {
  localStorage.setItem(PROJECTS_KEY, JSON.stringify(docs));
}

function nextId(docs: ProjectDoc[]) {
...
export default function ProjectView() {
  ...
  const insertTodo = () => {
    insertHTML(
      `<div style="display:flex;align-items:flex-start;gap:8px;margin:6px 0;padding:4px 0" data-type="todo">` +
      ...
      `<span style="outline:none;flex:1" contenteditable="true">Task item</span></div><br/>`
    );
  };

  const insertDivider = () => {
    insertHTML(`<br/><hr style="border:none;border-top:2px solid #e7e5e4;margin:12px 0"/><br/>`);
  };
  ...
  const handleCreate = () => {
    ...
    const doc: ProjectDoc = {
      ...
      content: "",
      bannerColor: "#6366f1",
      bannerGradient: BANNER_GRADIENTS[0].value,
  ...
  const handleRename = (id: number) => {
    ...
    const updated = projects.map(p =>
      p.id === id ? { ...p, title: editTitleVal.trim(), updatedAt: new Date().toISOString() } : p
    );
    saveProjects(updated);
    setProjects(updated);
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 bg-[#fafaf8]">
        ...
        <div className="text-center">
          ...
          <p className="text-stone-500 text-sm">Create your first project document</p>
        </div>
        <button
        ...
        {showNewModal && (
          <NewProjectModal
            ...
            onClose={() => { setShowNewModal(false); setNewTitle(""); }}
          />
        )}
      </div>
    );
  ...
  const bannerStyle = activeProject?.bannerImg
    ...
    : { background: activeProject?.bannerGradient ?? "linear-gradient(135deg,#667eea,#764ba2)" };

  return (
    <div className="flex-1 flex min-h-0 overflow-hidden bg-[#fafaf8]">

      {/* ── Left sidebar: project list ── */}
      <div className="w-52 flex-shrink-0 bg-white border-r border-stone-200 flex flex-col overflow-hidden">
        <div className="px-3 py-3 border-b border-stone-100 flex items-center justify-between flex-shrink-0">
          <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Projects</span>
          <button
            onClick={() => setShowNewModal(true)}
            className="w-6 h-6 rounded-md flex items-center justify-center text-stone-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all"
            title="New project"
          >
            ...
            <div key={p.id} className="group relative">
              {editingTitleId === p.id ? (
                <div className="flex items-center gap-1 px-1 py-1">
                  <input
                    ...
                    onKeyDown={e => { if (e.key === "Enter") handleRename(p.id); if (e.key === "Escape") setEditingTitleId(null); }}
                    className="flex-1 text-xs border border-indigo-300 rounded-md px-2 py-1 outline-none min-w-0"
                  />
                  <button onClick={() => handleRename(p.id)} className="text-green-600 hover:text-green-700"><Check className="w-3.5 h-3.5" /></button>
                </div>
              ) : (
                <button
                  onClick={() => { setActiveId(p.id); }}
                  className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg text-left transition-all ${
                    activeId === p.id ? "bg-indigo-50 text-indigo-700" : "text-stone-600 hover:bg-stone-50"
                  }`}
                >
                  <FileText className={`w-3.5 h-3.5 flex-shrink-0 ${activeId === p.id ? "text-indigo-500" : "text-stone-400"}`} />
              ...
              {editingTitleId !== p.id && (
                <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-0.5">
                  <button
                    onClick={(e) => { e.stopPropagation(); setEditingTitleId(p.id); setEditTitleVal(p.title); }}
                  ...
                  {deleteConfirmId === p.id ? (
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(p.id); setTimeout(() => setDeleteConfirmId(null), 3000); }}
          ...
          <>
                    ...
                    <div className="absolute bottom-10 right-0 w-80 bg-white rounded-xl shadow-2xl border border-stone-200 p-4 z-50">
                      ...
                      <div className="grid grid-cols-5 gap-1.5 mb-3">
                        {BANNER_GRADIENTS.map(g => (
                          <button
                            key={g.id}
                            onClick={() => setBanner({ bannerGradient: g.value, bannerImg: undefined })}
                      ...
                      <div className="grid grid-cols-3 gap-1.5 mb-3">
                        {BANNER_IMAGES.map(img => (
                          <button
                            key={img.id}
                            onClick={() => setBanner({ bannerImg: img.url, bannerGradient: undefined })}
            ...
            <div className="px-12 pt-6 pb-2 flex-shrink-0">
              <div
                ...
                className="text-4xl font-bold text-stone-900 outline-none empty:before:content-['Untitled_Project'] empty:before:text-stone-300 w-full"
            ...
            <div className="px-12 py-1 flex-shrink-0">
              <p className="text-[11px] text-stone-400 select-none">
                Right-click anywhere in the editor for formatting options · Ctrl+B Bold · Ctrl+I Italic · Ctrl+U Underline
              </p>
            </div>
      ...
      {ctxMenu && (
        <div
          ref={ctxMenuRef}
          className="fixed z-[9999] bg-white rounded-xl shadow-2xl border border-stone-200 py-1.5 min-w-[200px]"
              ...
              <div className="absolute left-full top-0 ml-1 bg-white rounded-xl shadow-2xl border border-stone-200 p-2.5 flex gap-1.5 z-[10000]">
                {HIGHLIGHT_COLORS.map(hc => (
                  <button
                    key={hc.color}
                    title={hc.label}
                    onClick={() => execFmt("hiliteColor", hc.color)}
                ...
                <button
                  ...
                  onClick={() => execFmt("hiliteColor", "transparent")}
...
function CtxItem({
  icon, label, shortcut, hasArrow, onClick
}: {
...
function NewProjectModal({ value, onChange, onCreate, onClose }: {
  value: string;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-80 p-6">
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2 rounded-xl border border-stone-200 text-sm text-stone-600 hover:bg-stone-50 transition-all">Cancel</button>
          <button onClick={onCreate} disabled={!value.trim()} className="flex-1 py-2 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition-all disabled:opacity-40">Create</button>
        </div>
      </div>
    </div>
  );
