  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-10">
      <div className="w-full max-w-[820px] space-y-2">
        <p className="text-[10px] font-bold tracking-widest text-stone-300 uppercase mb-4">Flat C — Priority Row Tint + Left Bar</p>
        {tasks.map((task, idx) => {
          const pm = task.priority ? pmeta[task.priority] : null;
          return (
            <div
              key={task.id}
              className="flex items-center gap-0 border border-stone-200 rounded-sm overflow-hidden"
              style={pm ? { backgroundColor: pm.rowBg } : { backgroundColor: "white" }}>

              {/* Priority accent bar */}
              <div className="w-[3px] self-stretch flex-shrink-0" style={pm ? { backgroundColor: pm.bar } : { backgroundColor: "#e5e7eb" }} />

              <div className="border-r border-stone-200 px-3 py-3 flex-shrink-0">
                <span className="text-[11px] font-semibold text-stone-500">T{idx + 1}</span>
              </div>

              <div className="border-r border-stone-200 px-3 py-3 flex-shrink-0 flex items-center justify-center">
                <div
              ...
              <div className="flex-1 border-r border-stone-200 px-3 py-3">
                <input
                  readOnly
                  value={task.done ? "" : task.title}
                  placeholder={task.done ? task.title : "Task"}
                  className={`w-full text-sm outline-none bg-transparent border-b pb-0.5 placeholder-stone-300 ${task.done ? "line-through text-stone-300 border-stone-100" : "text-stone-700 border-stone-300"}`}
                />
              </div>

              <div className="border-r border-stone-200 px-4 py-3 flex-shrink-0">
                <span className="text-[11px] text-stone-500">Note</span>
              </div>

              <div className="border-r border-stone-200 px-4 py-3 flex-shrink-0 min-w-[90px] text-center">
                <span className={`text-[11px] tabular-nums ${!task.time ? "text-stone-300" : "text-stone-600"}`}>
                  {task.time || "--:-- --"}
                </span>
              </div>

              <div className="border-r border-stone-200 px-4 py-3 flex-shrink-0 min-w-[80px] text-center">
